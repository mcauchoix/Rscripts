...
DTA=DT[,list(ACC =mean(door.open),site=unique(site_folder),species=unique(species),elevation=unique(elevation),scenario=unique(scenario),nbtrial = .N), by="day,tag"]
...
DTI=DT[,list(ACC =mean(ACC),nind=length(unique(tag)),esACC=sd(ACC)/length(unique(tag))*1.96,nbtrial =mean(nbtrial),esNBT=sd(nbtrial)/length(unique(tag))*1.96), by="day,site_folder,scenario"]
DTS=DT[,list(ACC =mean(ACC),nind=length(unique(tag)),sdAcc=sd(ACC),esACC=sd(ACC)/length(unique(tag))*1.96,nbtrial =mean(nbtrial),esNBT=sd(nbtrial)/length(unique(tag))*1.96), by="day,site_folder,species,scenario"]
...
for (i in 1:length(uSc)){
  ...
  for (s in 1:length(uSite)){
    ...
    dta$daynumber[ind]=dta$dayDate[ind]-min(dta$dayDate[ind])+1
    for (t in 1:length(uTag)){
      indi=ind&dta$tag==uTag[t]
      if (sum(indi)>0){
      dta$BirdDaynumber[indi]=1:sum(indi)
      }
    }
  }
}
...
DTAS=DT[nbtrial>1,list(ACC =mean(ACC),esACC=sd(ACC)/length(unique(tag))*1.96,nbtrial =mean(nbtrial),esNBT=sd(nbtrial)/length(unique(tag))*1.96), by="BirdDaynumber,species,scenario"]#daynumber
DTASI=DT[,list(ACC =mean(ACC),esACC=sd(ACC)/length(unique(tag))*1.96,nbtrial =mean(nbtrial),esNBT=sd(nbtrial)/length(unique(tag))*1.96), by="BirdDaynumber,elevation,scenario"]
...
# By SPECIES
#----------
p30=ggplot(subset(dtas,scenario==30), aes(x = BirdDaynumber, y = ACC)) +
  ...
  xlab("Bird Day Number") + ylab("Daily Accuracy")

p31=ggplot(subset(dtas,scenario==31), aes(x = BirdDaynumber, y = ACC)) +
  geom_line(aes(colour=species)) + 
  geom_pointrange(aes(ymin = ACC - esACC,
                      ymax = ACC + esACC,colour=species))+
  xlim(1,14)+
  ...
  xlab("Bird Day Number") + ylab("")

p32=ggplot(subset(dtas,scenario==32), aes(x = BirdDaynumber, y = ACC)) +
  geom_line(aes(colour=species)) + 
  geom_pointrange(aes(ymin = ACC - esACC,
                      ymax = ACC + esACC,colour=species))+
  xlim(1,14)+
...
# save graph
pdf(paste0(out_ls,"Learning population by spe.pdf"), height=6, width=12)
grid.arrange(p30,p31, nrow=1, ncol=2)#grid.arrange(p30,p31,p32, nrow=1, ncol=3)
dev.off()

# BY ELEVATION
#-------------
p30=ggplot(subset(dtasi,scenario==30), aes(x = BirdDaynumber, y = ACC)) +
...
p31=ggplot(subset(dtasi,scenario==31), aes(x = BirdDaynumber, y = ACC)) +
  geom_line(aes(colour=elevation)) + 
  geom_pointrange(aes(ymin = ACC - esACC,
                      ymax = ACC + esACC,colour=elevation))+
  xlim(1,14)+
...
p32=ggplot(subset(dtasi,scenario==32), aes(x = BirdDaynumber, y = ACC)) +
  geom_line(aes(colour=elevation)) + 
  geom_pointrange(aes(ymin = ACC - esACC,
                      ymax = ACC + esACC,colour=elevation))+
  xlim(1,14)+
